...
chmod u+w nci-eventsreg/web/sites/default
rm -rf nci-eventsreg
chmod u+w nci-cct-eventsreg/web/sites/default

echo "Cloning drupal 8 from github"
git clone https://github.com/cbiit/nci-eventsreg nci-eventsreg
cd nci-eventsreg
...
composer require drupal/webform_views:^5.0 dompdf/dompdf:^2.0.0 drupal/entity_print:^2.7 drupal/default_content_deploy:^2.0 drupal/default_content:^2.0@alpha drupal/ldap cweagans/composer-patches drupal/core-recommended:9.4.8 drupal/module_missing_message_fixer drush/drush:^10.0.0 drupal/backup_migrate:^5 drupal/core-composer-scaffold:9.4.8 drupal/core-project-message:9.4.8 --update-with-dependencies --no-update
composer update
drush updb -y
